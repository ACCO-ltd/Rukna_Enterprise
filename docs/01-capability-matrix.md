...
| Document register / revisions / transmittals | ✓ | ✓ | PARTIAL | Standalone project **document register** (`ProjectDocument`: category + title on a PlatformFile) + Documents tab shipped; its files are now bound and discarded with the document. **Corrected 2026-09-07:** linked documents is blocked by **five unwritten upload paths**, not by the missing `GET /projects/:id/linked-documents` — `Contract`/`Guarantee`/`Ipa`/`Ipc`/`JournalEntry` attachment tables have no service or controller (only `DprAttachment` is wired), so an aggregation endpoint would return an empty list forever. Classification in `docs/design/documents-phase7-foundation.md` §7. No revisions, no expiry, no status, no org-level register — designed but not built (§8–§9); expiry/revision semantics need Eng Ahmed. |

...
| Capability | Backend | Frontend | Status | Notes |
|---|---|---|---|---|
| Projects (lifecycle, suspend/resume, members) | ✓ | ✓ | INTEGRATED | ADR-019 six-state guarded lifecycle (Phase A collapse: DRAFT/ACTIVE/PRACTICAL_COMPLETION/CLOSEOUT/CLOSED/CANCELLED; DRAFT shown as "Preparation"); suspend/resume as a condition; versioned members. **ADR-019 Phase B1 (CONST-PLC-005/009): queryable readiness read contract** `GET /projects/:id/readiness?command=` — `ProjectReadinessPolicy` returns per-command MANDATORY/WAIVABLE conditions (rich for `start`; close/closeout deferred until their source domains expose state). Deferred: Phase B2 (enforce readiness + waivers + evidence payloads CONST-PLC-006/008). `construction/projects` |

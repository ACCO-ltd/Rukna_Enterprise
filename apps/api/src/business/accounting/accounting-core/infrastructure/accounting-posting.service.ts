  ...
  async post(command: PostingCommand, tx: TxClient): Promise<PostingResult> {
    ...
    const resolvedLines = await Promise.all(
      command.lines.map(async (line, idx) => {
        ...
        return {
          ...
          transactionAmount: line.transactionAmount ?? null,
          baseCurrencyAmount: line.baseCurrencyAmount,
          exchangeRateSnapshot: line.exchangeRateSnapshot ?? null,
    ...
    const entry = await tx.journalEntry.create({
      data: {
        ...
        exchangeRateSnapshot: command.exchangeRateSnapshot ?? null,
